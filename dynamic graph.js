...
var svg = d3.select("body").append("svg")
	...
	.append("g")
		.attr("transform", "translate(" + 0.1*m[3] + "," + m[0] + ")");
...
$.getJSON("data/RecordSector2.json",function(rjsondata){
		jsondata = rjsondata;
		})
function getprofile(){
...
var End = $('#EndDate').val();
data1 = $.grep(jsondata,function(e) {return ( e.Aimcompany===Company && e.date<=Number(End) && e.date>=Number(Start));});
...
function lines() {
	...
	y = d3.scale.linear().range([h / number_symbol - 20, 0]);
	...
	var g = svg.selectAll(".symbol")
			.attr("transform", function(d, i) { return "translate(0," + (i * h / number_symbol + 10) + ")"; });
...
function horizons() {
	svg.insert("defs", ".symbol")
		...
		.append("rect")
			.attr("width", w)
			.attr("height", h / number_symbol - 20);
	...
	area
			.y0(h / number_symbol - 20);

	g.select("circle").transition()
			.duration(0)
			.attr("transform", function(d) { return "translate(" + (w - 200) + "," + (-h / number_symbol) + ")"; })
			.remove();

	g.select("text").transition()
			.duration(0)
			.attr("transform", function(d) { return "translate(" + (w - 200) + "," + (h / number_symbol - 30) + ")"; })
			.attr("dy", "0em");
	...
	g.each(function(d) {
		...
		d3.select(this).selectAll(".area")
			...
			.enter().insert("path", ".line")
				.attr("class", "area")
				.attr("transform", function(d) { return "translate(0," + (d * (h / number_symbol - 10)) + ")"; })
...
function stackedArea() {
	...
	t.select("text")
			.attr("transform", function(d) { d = d.values[d.values.length - 1]; return "translate(" + (w - 200) + "," + y(d.price / 2 + d.price0) + ")"; });
...
function overlappingArea() {
	...
	t.select("text")
			.attr("dy", ".31em")
			.attr("transform", function(d) { d = d.values[d.values.length - 1]; return "translate(" + (w - 200) + "," + y(d.price) + ")"; });
...
$('#submit').click(function() {
	...
	number_symbol = _.uniq(data2.map(a=>a.symbol)).length

	for (var i = 0, len = data2.length; i < len; i++) {
data2[i].date = data2[i].date.toString();
};
	updateRawData(data2);
})
